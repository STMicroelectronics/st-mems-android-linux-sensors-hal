...
SWGyroLimitedAxesUncalibrated::SWGyroLimitedAxesUncalibrated(const char *name, int handle, int module)
                       : SWSensorBaseWithPollrate(name, handle,
                                                  GyroSensorLimitedAxisUncalibratedType,
                                                  true, true, true, true, module)
{
    dependencies_type_list.push_back(GyroSensorLimitedAxisType);
    id_sensor_trigger = SENSOR_DEPENDENCY_ID_0;
    sensor_event.data.dataLen = 9;
}

void SWGyroLimitedAxesUncalibrated::ProcessData(SensorBaseData *data)
{
    int axes;
